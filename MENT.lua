-- ============================================
-- ANTI-DETECTION AUTO FARM (TweenService)
-- ============================================
...
local Settings = {
    ...
    SearchRange = 500,
    MoveSpeed = 0.3,         -- Movement speed (seconds)
    AutoFarm = false,
}
...
Frame.Position = UDim2.new(0.01, 0, 0.5, -95)
...
-- ANTI-DETECTION: Smooth movement using TweenService
function MoveToMobSmooth(Mob)
    ...
    -- ANTI-DETECTION: Use TweenService (same as old code)
    local TweenService = game:GetService("TweenService")
    local TweenInfo = TweenInfo.new(
        Settings.MoveSpeed,
        Enum.EasingStyle.Linear
    )
    Tween = TweenService:Create(PlayerHRP, TweenInfo, { CFrame = TargetCFrame })
    Tween:Play()
    Tween.Completed:Wait()
    Tween = nil
end

-- ANTI-DETECTION: Sticky position update (smooth)
function UpdateStickyPosition()
    ...
    if Distance > 0.5 then
        -- ANTI-DETECTION: Small smooth movement
        local TweenService = game:GetService("TweenService")
        local TweenInfo = TweenInfo.new(
            0.1,  -- Very fast but still smooth
            Enum.EasingStyle.Linear
        )
        ...
        while isFarming do
            if not IsTargetAlive(CurrentTarget) then
                ...
                if CurrentTarget then
                    ...
                    -- ANTI-DETECTION: Smooth move to target
                    MoveToMobSmooth(CurrentTarget)
                else
                    TargetStatus.Text = "Target: None"
                    TargetStatus.TextColor3 = Color3.fromRGB(200, 200, 200)
            ...
            -- ANTI-DETECTION: Update position smoothly
            ...
            task.wait(0.05) -- Fast but smooth updates
        end
